...
impl FromStr for AptosChainReference {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let chain_id = s.parse::<u8>().map_err(|_| {
            AptosChainReferenceFormatError::InvalidReference(s.to_string())
        })?;
        if chain_id != 1 && chain_id != 2 {
            return Err(AptosChainReferenceFormatError::InvalidReference(
                format!("Invalid Aptos chain ID: {}", chain_id),
            ));
        }
        Ok(Self(chain_id))
    }
...
impl From<AptosChainProviderError> for X402SchemeHandlerError {
...
impl AptosChainProvider {
    pub async fn from_config(
        ...
        let chain = AptosChainReference::from_str(&config.network())?;
        ...
        let rest_client = AptosClient::new(url::Url::parse(rpc_url)?);
...
impl FromStr for Address {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let address = AccountAddress::from_str(s)
            .map_err(|e| format!("Invalid Aptos address: {}", e))?;
        Ok(Self(address))
    }
}
